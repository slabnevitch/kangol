...
var siblings = function siblings(elem) {
  // принимает только объекты, НЕ селекторы!
...
function singleDropdown() {
  ...
  docClick = function docClick(e) {
    ...
    if (window.innerWidth > 960 && isMobile.any()) {
      // определяем, что клик с тач-скрина
      if (targetEl.hasAttribute('data-single-dropdown-link')) {
      ...
      if (!targetEl.closest('[data-single-dropdown]') && document.querySelector('[data-single-dropdown].touch-hover') !== null) {
  ...
  if (isMobile.any()) {
    ...
    for (var i = 0; i < arrows.length; i++) {
      arrows[i].addEventListener('click', function (e) {
        ...
        Array.prototype.slice.call(parent.querySelectorAll('[data-dropdowned]')).forEach(function (item) {
...
function ScrollTabs() {
  var _this = this;
  var tabsContainer = document.querySelector('[data-scroll-tabs]'),
  ...
  this.init = function () {
    _this.events();
    _this.arrowsSwitcher();
    // this.updateIcons();
    _this.updateIconsOnLoad();
    console.log('ScrollTabs!');
  };
  this.scrollbarCheck = function (element) {
    return element.scrollWidth > element.clientWidth || element.scrollHeight > element.clientHeight;
  };
  this.arrowsSwitcher = function () {
    // console.log(this.scrollbarCheck(tabsContainer));
    tabsArrows.forEach(function (arrow) {
      return _this.scrollbarCheck(tabsContainer) ? arrow.classList.remove('hidden') : arrow.classList.add('hidden');
    });
    // this.updateIcons();
  };
  this.events = function () {
    tabsItems.forEach(function (tabsItem) {
      return addEventListener('click', _this.tabClick);
    });
    tabsArrows.forEach(function (tabsArrow) {
      return addEventListener('click', _this.arrowClick);
    });
    tabsContainer.addEventListener('scroll', _this.tabsScroll);
    tabsContainer.addEventListener('wheel', _this.tabsWhell);
  ...
  this.tabsScroll = function (e) {
    return _this.updateIcons(e.target.scrollLeft);
  };
  this.updateIcons = function (scrolledWidth) {
  ...
  this.updateIconsOnLoad = function (scrolledWidth) {
    // console.log(scrolledWidth <= 1)
...
(function () {
  var detailsSpoilers = function detailsSpoilers(minimizeSiblings) {
    ...
    closeAllSpoilers = function closeAllSpoilers(translatedSpoiler) {
    ...
    openAllSpoilers = function openAllSpoilers(translatedSpoiler) {
      var currentSpoiler = translatedSpoiler;
      if (currentSpoiler) {
        Array.prototype.slice.call(currentSpoiler.querySelectorAll('[data-spoiler-item]')).forEach(function (spoilerItem) {
    ...
    mediaQuerySpy = function mediaQuerySpy() {
    ...
    mediaQueryChange = function mediaQueryChange(e) {
      if (e.matches) {
        console.log('matches');
    ...
    selectCurrentSpoilers = function selectCurrentSpoilers(condition) {
...
!function (t, e) {
  ...
  function Pi(t, e) {
    ...
    var _r2 = function r() {
      ...
      return _r2.cache = o.set(n, s), s;
    };
    return _r2.cache = new (Pi.Cache || ji)(), _r2;
  }
  function Fi(t) {
    var e = _typeof(t);
    return !!t && ("object" == e || "function" == e);
...
(function (element) {
  ...
  element.closest = element.closest || function closest(selector) {
    if (!this || !this.parentElement && !this.matches(selector)) return null;else if (this.matches(selector)) return this;else return this.parentElement.closest(selector);
  };
})(Element.prototype);
function setFstDropdown() {
  var list = document.querySelectorAll(".fstdropdown-select:not(.fstcreated)");
  for (var sel in list) if (list.hasOwnProperty(sel)) createDropdown(list[sel]);
  function createDropdown(select) {
    ...
    var dropdown = createFstElement("div", "fstdropdown" + (opened != null && opened == "true" ? " open" : ""), div, opened == null || opened != "true" ? {
      "click": openSelect,
      "blur": openSelect
    } : null);
    dropdown.select = select;
    dropdown.setAttribute("tabindex", "0");
    if (opened == null || opened != "true") createFstElement("div", "fstselected", dropdown, null);
    ...
    if (select.multiple) {
      var selectAll = createFstElement("button", "fstAll", dropdown, {
  ...
  function openSelect(event, open, force) {
    ...
    if (select.dataset["searchdisable"] == null && select.dataset["searchdisable"] != "true") el.querySelector(".fstsearchinput").focus();
  }
  ...
  function changeSelect(event) {
    ...
    if (select.value != event.target.dataset["value"] && !select.multiple) {
      ...
      if (dd.querySelector(".fstlist>.selected") != null) dd.querySelector(".fstlist>.selected").classList.remove("selected");
      event.target.classList.add("selected");
      if (opened) dd.querySelector(".fstselected").textContent = event.target.textContent;
      initNewEvent("change", select);
    }
    if (select.multiple) {
      changeMultipleSelect(select, event, dd, selectAll, opened);
    }
    if (opened) openSelect(event, false);
  }
  function changeMultipleSelect(select, event, dd, selectAll, opened) {
    ...
    if (event.target.classList.contains("selected")) event.target.classList.remove("selected");else event.target.classList.add("selected");
    initNewEvent("change", select);
    var length = dd.querySelectorAll(".fstlist>.selected").length;
    selectAll.selected = length > 0;
    if (opened) dd.querySelector(".fstselected").textContent = length == 1 ? event.target.textContent : length + " options selected";
  ...
  function rebindDropdown(select) {
    ...
    while (ddList.lastChild) ddList.removeChild(ddList.lastChild);
    for (var opt in optList) {
      if (optList.hasOwnProperty(opt)) {
        ...
        if (optList[opt].selected) listEl.classList.add("selected");
  ...
  function getSearch(event) {
    ...
    for (var div in ddList) if (ddList.hasOwnProperty(div)) if (ddList[div].textContent.trim().toLowerCase().indexOf(val.trim().toLowerCase()) != -1) ddList[div].classList.remove("hideFst");else ddList[div].classList.add("hideFst");
  }
  function createFstElement(type, className, parent, eventListener) {
    var element = document.createElement(type);
    if (className != null) for (var c in className.split(" ")) if (!isNaN(c)) element.classList.add(className.split(" ")[c]);
    if (eventListener != null) for (var ev in eventListener) if (eventListener.hasOwnProperty(ev)) element.addEventListener(ev, eventListener[ev], true);
  ...
  function selectAllOptions(event) {
    ...
    for (var l in list) if (list.hasOwnProperty(l)) {
      select.querySelector("[value='" + list[l].dataset["value"] + "']").selected = selected;
      if (selected) list[l].classList.add("selected");else list[l].classList.remove("selected");
    }
    initNewEvent("change", select);
  ...
  function setHeader(select, event) {
    if (select.dataset["opened"] == null || select.dataset["opened"] != "true") {
      ...
      if (select.multiple) {
        var count = 0;
        for (var s in select.options) if (select.options.hasOwnProperty(s) && select.options[s].selected == true) count++;
      ...
      if (event != null) openSelect(event, false, true);
    }
  }
  return {};
}
...
// EXTERNAL MODULE: ./app/js/vendor/service-functions/all-functions.js
var all_functions = __webpack_require__(1);

// EXTERNAL MODULE: ./node_modules/jquery/dist/jquery.js
var jquery = __webpack_require__(0);
var jquery_default = /*#__PURE__*/__webpack_require__.n(jquery);

// CONCATENATED MODULE: ./app/js/source/mob-footer-menu.js

window.jQuery = jquery_default.a;
window.$ = jquery_default.a;
var mob_footer_menu_mobFooterMenu = function mobFooterMenu() {
  function bodyLock(isLock) {
    if (isLock) jquery_default()('html').addClass('lock');else jquery_default()('html').removeClass('lock');
  }
  jquery_default()(".mob-menu-trigger").click(function (e) {
    e.stopPropagation();
    jquery_default()(".catatalog__select .fstdiv").removeClass('open');
    jquery_default()(".catatalog__select .fstdropdown").removeClass('open');
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(".modal-menu").hide();
      bodyLock(false);
    } else {
      jquery_default()(".modal-size").hide();
      jquery_default()(".modal-shops").hide();
      jquery_default()(".modal-sort").hide();
      jquery_default()(".mob-modal-view").hide();
      jquery_default()(".mob-modal").hide();
      jquery_default()(".modal-icon-block").removeClass("active");

      // $(this).addClass("active");
      jquery_default()(".modal-menu").show();
      bodyLock(true);
    }
  });
  jquery_default()(".modal-menu__close").click(function (e) {
    jquery_default()(".modal-menu").hide();
    bodyLock(false);
  });
  jquery_default()(".btn-list-auth .btn-1").click(function (e) {
    e.stopPropagation();
    jquery_default()(".btn-list-auth .btn").removeClass("active");
    jquery_default()(this).addClass("active");
    jquery_default()(".text-auth-1").show();
    jquery_default()(".text-auth-2").hide();
  });
  jquery_default()(".btn-list-auth .btn-2").click(function (e) {
    e.stopPropagation();
    jquery_default()(".btn-list-auth .btn").removeClass("active");
    jquery_default()(this).addClass("active");
    jquery_default()(".text-auth-2").show();
    jquery_default()(".text-auth-1").hide();
  });
  jquery_default()(".footer-icon-cart").click(function (e) {
    e.stopPropagation();
    jquery_default()(".catatalog__select .fstdiv").removeClass('open');
    jquery_default()(".catatalog__select .fstdropdown").removeClass('open');
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(".modal-cart").hide();
      bodyLock(false);
    } else {
      jquery_default()(".mob-menu-trigger").removeClass("active");
      jquery_default()(".modal-menu").hide();
      jquery_default()(".modal-search").hide();
      jquery_default()(".modal-size").hide();
      jquery_default()(".modal-shops").hide();
      jquery_default()(".modal-sort").hide();
      ...
      jquery_default()(".mob-modal").hide();
      jquery_default()(".modal-icon-block").removeClass("active");
      jquery_default()(".mob-search").removeClass("active");
      jquery_default()(this).addClass("active");
      jquery_default()(".modal-cart").show();
      bodyLock(true);
    }
  });
  jquery_default()(".modal-cart__close").click(function (e) {
    jquery_default()(".modal-cart").hide();
    jquery_default()(".footer-icon-cart").removeClass("active");
    bodyLock(false);
  });
  jquery_default()(".mob-search").click(function (e) {
    e.stopPropagation();
    jquery_default()(".catatalog__select .fstdiv").removeClass('open');
    jquery_default()(".catatalog__select .fstdropdown").removeClass('open');
    if (jquery_default()(this).hasClass("active")) {
      ...
      jquery_default()(".modal-icon-block").removeClass("active");
      jquery_default()(this).addClass("active");
      jquery_default()(".modal-search").show();
      bodyLock(true);
    }
  });
  jquery_default()(".modal-search__close").click(function (e) {
    e.stopPropagation();
    jquery_default()(".modal-search").hide();
    jquery_default()(".mob-search").removeClass("active");
    bodyLock(false);
  });
  jquery_default()(".footer-icon-fav").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(".modal-fav").hide();
    } else {
      jquery_default()(".mob-menu-trigger").removeClass("active");
      jquery_default()(".modal-menu").hide();
      jquery_default()(".modal-size").hide();
      ...
      jquery_default()(".modal-icon-block").removeClass("active");
      jquery_default()(this).addClass("active");
      jquery_default()(".modal-fav").show();
    }
  });
  jquery_default()(".mob-modal .modal-bg").click(function () {
    jquery_default()(".mob-menu-trigger").removeClass("active");
    jquery_default()(".modal-menu").hide();
    jquery_default()(".mob-search").removeClass("active");
    jquery_default()(".modal-icon-block").removeClass("active");
  ...
  jquery_default()(".footer-icon-account").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(".modal-auth").hide();
      jquery_default()(".modal-reg").hide();
    } else {
      jquery_default()(".mob-menu-trigger").removeClass("active");
      jquery_default()(".modal-menu").hide();
      jquery_default()(".modal-reg").hide();
      ...
      jquery_default()(".modal-icon-block").removeClass("active");
      jquery_default()(this).addClass("active");
      jquery_default()(".modal-auth").show();
    }
  });
  jquery_default()(".open-modal-auth-pass").click(function (e) {
    e.stopPropagation();
    jquery_default()(".mob-menu-trigger").removeClass("active");
    jquery_default()(".modal-menu").hide();
    jquery_default()(".modal-cart").hide();
    ...
    jquery_default()(".modal-auth").hide();
    jquery_default()(".modal-auth-pass").show();
  });
  jquery_default()(".modal-auth-pass .modal-bg").click(function (e) {
    e.stopPropagation();
    jquery_default()(".footer-icon-account").addClass("active");
    jquery_default()(".modal-auth-pass").hide();
    jquery_default()(".modal-auth").show();
  });
  jquery_default()(".open-reg").click(function (e) {
    e.stopPropagation();
    jquery_default()(".mob-menu-trigger").removeClass("active");
    jquery_default()(".modal-menu").hide();
    jquery_default()(".modal-cart").hide();
    ...
    jquery_default()(".modal-auth").hide();
    jquery_default()(".modal-reg").show();
  });
  jquery_default()(".open-restore-modal").click(function (e) {
    e.stopPropagation();
    jquery_default()(".mob-menu-trigger").removeClass("active");
    jquery_default()(".modal-menu").hide();
    jquery_default()(".modal-cart").hide();
    ...
    jquery_default()(".modal-auth").hide();
    jquery_default()(".modal-restore").show();
  });
  jquery_default()(".footer-icon-view").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(".mob-modal-view").hide();
      ...
      jquery_default()(".modal-icon-block").removeClass("active");
      jquery_default()(this).addClass("active");
      jquery_default()(".mob-modal-view").show();
    }
  });
  jquery_default()(".footer-icon-sort").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(".modal-sort").hide();
      ...
      jquery_default()(".modal-icon-block").removeClass("active");
      jquery_default()(this).addClass("active");
      jquery_default()(".modal-sort").show();
    }
  });
  jquery_default()(".mob-modal-view .view").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
    } else {
      jquery_default()(".mob-modal-view .view").removeClass("active");
      jquery_default()(this).addClass("active");
    }
  });
  jquery_default()(".footer-icon-shops").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(".modal-shops").hide();
      ...
      jquery_default()(".modal-size").hide();
      jquery_default()(".modal-sort").hide();
      jquery_default()(".footer-icon-view").removeClass("active");
      jquery_default()(".mob-modal-view").hide();
      jquery_default()(".mob-modal").hide();
      jquery_default()(".modal-icon-block").removeClass("active");
      jquery_default()(this).addClass("active");
      jquery_default()(".modal-shops").show();
    }
  });
  jquery_default()(".footer-icon-filter").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(".modal-filter").hide();
    } else {
      jquery_default()(".mob-menu-trigger").removeClass("active");
      jquery_default()(".modal-menu").hide();
      jquery_default()(".modal-size").hide();
      ...
      jquery_default()(".modal-icon-block").removeClass("active");
      jquery_default()(this).addClass("active");
      jquery_default()(".modal-filter").show();
    }
  });
  jquery_default()(".modal-size-trigger").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(".modal-size").hide();
      ...
      jquery_default()(".modal-icon-block").removeClass("active");
      jquery_default()(this).addClass("active");
      jquery_default()(".modal-size").show();
    }
  });
  jquery_default()(".mob-filter-title").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(this).next().slideUp();
    } else {
      jquery_default()(".mob-filter-title").removeClass("active");
      jquery_default()(".mob-filter-sub").slideUp();
      jquery_default()(this).addClass("active");
      jquery_default()(this).next().slideDown();
    }
  });
  jquery_default()(".mob-filter-item.selected").click(function (e) {
    e.stopPropagation();
    jquery_default()(this).removeClass("selected");
  });
  jquery_default()(".mob-circle-item").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
    } else {
  ...
  jquery_default()(".lk-field-block .link-change").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(this).parent().parent().removeClass("active");
      jquery_default()(this).html("Изменить");
    } else {
      jquery_default()(this).addClass("active");
      jquery_default()(this).html("Подтвердить");
      jquery_default()(this).parent().parent().addClass("active");
    }
  });
  jquery_default()(".lk-accordion .accordion-item:first-child .accordion-menu-title").addClass("active");
  jquery_default()(".lk-accordion .accordion-item:first-child .accordion-sub-block").show();
  jquery_default()(".lk-accordion .accordion-menu-title").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(this).next().slideUp();
    } else {
      jquery_default()(".lk-accordion .accordion-menu-title").removeClass("active");
      jquery_default()(".lk-accordion .accordion-sub-block").slideUp();
      jquery_default()(this).addClass("active");
      jquery_default()(this).next().slideDown();
    }
  });
  jquery_default()(".footer-acc .accordion-item:first-child .accordion-menu-title").addClass("active");
  jquery_default()(".footer-acc .accordion-item:first-child .accordion-sub-block").show();
  jquery_default()(".footer-acc .accordion-menu-title").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(this).next().slideUp();
    } else {
      jquery_default()(".footer-acc .accordion-menu-title").removeClass("active");
      jquery_default()(".footer-acc .accordion-sub-block").slideUp();
      jquery_default()(this).addClass("active");
      jquery_default()(this).next().slideDown();
    }
  });
  ...
  jquery_default()(".mob-menu-acc .accordion-menu-title").click(function (e) {
    e.stopPropagation();
    if (jquery_default()(this).hasClass("active")) {
      jquery_default()(this).removeClass("active");
      jquery_default()(this).next().slideUp();
    } else {
      jquery_default()(".mob-menu-acc .accordion-menu-title").removeClass("active");
      jquery_default()(".mob-menu-acc .accordion-sub-block").slideUp();
      jquery_default()(this).addClass("active");
      jquery_default()(this).next().slideDown();
    }
  });
};
// CONCATENATED MODULE: ./app/js/source/mob-common.js
var mobCommon = function mobCommon() {
  $(function () {
    function bodyLock(isLock) {
      if (isLock) $('html').addClass('lock');else $('html').removeClass('lock');
    }
    $('.lazy').lazy();
    if ($(this).scrollTop() > 170) {
      $('.mob-header-bottom').css({
        "display": ""
      });
...
// CONCATENATED MODULE: ./app/js/components/product-card.js
var availabilityBooking = function availabilityBooking() {
  $('.mob-card-availability-list__item-book').on('click', function () {
    $(this).closest('.mob-card-availability-list__item').next('.product__booking').toggleClass('active');
  });
};
var sizeChecking = function sizeChecking() {
...
var footerObserver = function footerObserver() {
...
// CONCATENATED MODULE: ./app/js/components/checkout.js
var checkoutLabels = function checkoutLabels() {
...
// CONCATENATED MODULE: ./app/js/components/form.js
var inputReset = function inputReset() {
  $('.reset, .inp-reset').on('click', function (e) {
    $(this).prev('.form__input').val('');
  });
};
// CONCATENATED MODULE: ./app/js/components/main-slider.js
...
if (document.querySelectorAll('[data-single-dropdown]') != null) {
  Object(all_functions["singleDropdown"])();
}
...
var SimpleBar = __webpack_require__(5);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if ($('.mob-modal-box').length > 0) {
    if (all_functions["isMobile"].iOS()) {
      // alert(document.querySelectorAll('.mob-modal-box').length)
      window.addEventListener('resize', function () {
        // alert('resize')
        document.querySelectorAll('.mob-modal-box').forEach(function (item) {
          item.style.setProperty('--height', "".concat(window.innerHeight, "px"));
        });
      });
    }
  }
  ...
  if (!all_functions["isMobile"].any()) {
    if (document.getElementById('cards-filter-open') !== null && document.getElementById('cards-filter-select') !== null) {
      ...
      document.getElementById('cards-filter-open').onclick = function () {
    ...
    if (document.querySelector('.catatalog__header') !== null) {
      document.querySelector('.catatalog__header').classList.add('select-native');
    }
  }
  ...
  if (document.querySelector('.view-product__thumbs') !== null && document.querySelector('.view-product__images') !== null) {
    new all_functions["ScrollToSects"]({
      linksContainer: '.view-product__thumbs',
      //контейнер, в котором лежат кнопки навигации. Если контейнеров несколько, перечислить ч/з запятую.
      ...
      offset: document.querySelector('.view-product__image').offsetHeight / 3,
